...
extern u8 scopy[400 * 240]; //handle last line overflow
namespace sg{
	namespace gr{
		...
		public:
			...
			CImg(char *filename, bool flip=0){
				...
				if(!f) return ;
				//+36
				pData = new u8[400*240*3];
				pOrig = new u8[400*240*3];
				fread(pData,0x36,1,f);
				fread(pData,400*240*3,1,f);
				fclose(f);
				u8 r,g,b;
				if (flip){
					u8 *pOrigc=pOrig;
					...
					memcpy(pData,pOrig,400*240*3);
				}
				else{
				memcpy(pOrig,pData,400*240*3);
				}
			}
			CImg(u8 type){
...
outside:
				...
				for(int y=0;y<240;y++){
					for(int x=0;x<400;x++){
						...
						pData[(y*400 + x )*3+ 0] = (c>>8)&0xff;
						pData[(y*400 + x )*3+ 1] = (c>>16)&0xff;
						pData[(y*400 + x )*3+ 2] = (c>>0)&0xff;						
					}
				}
				memcpy(pOrig,pData,400*240*3);
			}
...
#define GetPixel2(x,y) copy[(y)*400 + x]
#define SetPixel2(x,y,v)  copy[(y)*400 + x]=v
#define UpdatePixel2(x,y,v) copy[(y)*400 + x]= copy[(y)*400 + x] + v


			void draw_steinberg(CSurface *s, u16 xx, u16 yy){
				//make a local copy
				u16 copy[400*242]; //handle last line overflow
				u16 *p = copy;

				i16 oldpix, newpix,error;
				for(int y=0;y<240;y++) for(int x=0;x<400;x++) *p++=GetPixel(x,y)&0xff;
				for(int y=0;y<240;y++){
					for(int x=0;x<400;x++){
...
#define rGetPixel2(x,y) rcopy[(y)*400 + x]
#define rSetPixel2(x,y,v)  rcopy[(y)*400 + x]=v
#define rUpdatePixel2(x,y,v) rcopy[(y)*400 + x]= rcopy[(y)*400 + x] + v

#define gGetPixel2(x,y) gcopy[(y)*400 + x]
#define gSetPixel2(x,y,v)  gcopy[(y)*400 + x]=v
#define gUpdatePixel2(x,y,v) gcopy[(y)*400 + x]= gcopy[(y)*400 + x] + v

#define bGetPixel2(x,y) bcopy[(y)*400 + x]
#define bSetPixel2(x,y,v)  bcopy[(y)*400 + x]=v
#define bUpdatePixel2(x,y,v) bcopy[(y)*400 + x]= bcopy[(y)*400 + x] + v


            void draw_steinbergcolor(CSurface *s, u16 xx, u16 yy){
                //make a local copy
                u16 rcopy[400 * 242]; //handle last line overflow
                u16 gcopy[400 * 242]; //handle last line overflow
                u16 bcopy[400 * 242]; //handle last line overflow
                u16 *pr = rcopy;
                u16 *pg = gcopy;
                u16 *pb = bcopy;
                ...
                for (int y = 0; y<240; y++) for (int x = 0; x<400; x++) *pr++ = GetPixel(x, y) & 0xff;
                for (int y = 0; y<240; y++) for (int x = 0; x<400; x++) *pg++ = (GetPixel(x, y)>>16) & 0xff;
                for (int y = 0; y<240; y++) for (int x = 0; x<400; x++) *pb++ = (GetPixel(x, y)>>8) & 0xff;
			...
			void draw_jarvis(CSurface *s, u16 xx, u16 yy){
				//make a local copy
				u16 copy[400*244]; //handle last line overflow
				u16 *p = copy;
            ...
            void draw_burkescolor(CSurface *s, u16 xx, u16 yy){
                ...
                i16 goldpix, gnewpix, gerror;
                i16 boldpix, bnewpix, berror;
                for (int y = 0; y<240; y++) for (int x = 0; x<400; x++) *rp++ = GetPixel(x, y) & 0xff;
                for (int y = 0; y<240; y++) for (int x = 0; x<400; x++) *gp++ = (GetPixel(x, y)>>16) & 0xff;
                for (int y = 0; y<240; y++) for (int x = 0; x<400; x++) *bp++ = (GetPixel(x, y)>>8) & 0xff;
                for (int y = 0; y<240; y++){
                    for (int x = 0; x<400; x++){
                        roldpix = rGetPixel2(x, y);
                        goldpix = gGetPixel2(x, y);
